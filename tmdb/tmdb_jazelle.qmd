...
search_movie_extra <- function(movie_id) {
  ...
  result <- data.frame(
    ...
    belongs_to_collection = ifelse(!is.null(json_data$belongs_to_collection), json_data$belongs_to_collection, NA),
    budget = json_data$budget,
    genres = ifelse(!is.null(json_data$genres), paste(sapply(json_data$genres$name, function(x) x), collapse = ", "), NA),
    ...
    production_companies = ifelse(!is.null(json_data$production_companies), paste(sapply(json_data$production_companies$name, function(x) x), collapse = ", "), NA),
    production_countries = ifelse(!is.null(json_data$production_countries), paste(sapply(json_data$production_countries$name, function(x) x), collapse = ", "), NA),
    release_date = json_data$release_date,
    revenue = json_data$revenue,
    runtime = json_data$runtime,
    spoken_languages = ifelse(!is.null(json_data$spoken_languages), paste(sapply(json_data$spoken_languages$name, function(x) x), collapse = ", "), NA),
...
```{r}
movie_details_extra <- apply(movie_details_df, 1, function(row) {
  search_movie_extra(row["id"])  
})

# Combine the results into one data frame
movie_details_df_extra <- do.call(rbind, movie_details_extra)

# Print the combined movie details data frame
print(movie_details_df_extra)
